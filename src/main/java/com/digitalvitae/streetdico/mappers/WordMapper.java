...
public class WordMapper {
    public Word toEntity(WordDto dto){
        return Word.builder().id(dto.getId()).name(dto.getName()).definition(dto.getDefinition()).build();
    }

    public WordDto toDto(Word entity){
        return WordDto.builder().id(entity.getId()).name(entity.getName()).definition(entity.getDefinition()).build();
    }
}
